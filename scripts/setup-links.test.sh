...
ln -s "$foreign_target" "$links/css-worker.md"
ln -s "$ROOT/agents/docs.md" "$links/docs.md"
...
if [[ "$(readlink "$links/css-worker.md")" != "$foreign_target" ]]; then
  printf 'foreign symlink was retargeted\n' >&2
  exit 1
fi
if [[ "$(readlink "$links/docs.md")" != "$ROOT/agents/docs.md" ]]; then

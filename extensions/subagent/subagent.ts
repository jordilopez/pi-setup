 ...
 *
 * Supports three modes (all in this one tool):
...
import type { DisplayItem, Mode, SingleResult, SubagentDetails } from "./types.ts";
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (let i = 0; i < params.chain.length; i++) {
          ...
          const chainUpdate = onUpdate
            ? (partial: Parameters<NonNullable<typeof onUpdate>>[0]) => {
